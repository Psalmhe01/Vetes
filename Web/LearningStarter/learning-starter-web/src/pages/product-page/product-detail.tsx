import { Container, Text, Skeleton, Grid, Card, Badge, Table, Anchor, Breadcrumbs, Button, Modal, TextInput, NumberInput, Select } from "@mantine/core";
import { PageWrapper } from "../../components/page-wrapper/page-wrapper";
import { useLocation, useNavigate, useParams } from "react-router-dom";
import { useEffect, useState } from "react";
import {ApiResponse, ProductGetDto, SizeGetDto, MeasurementTypeGetDto, ProductSizeCreateDto, ProductSizeMeasurementCreateDto} from "../../constants/types";
...
export const ProductDetail = () => {
  ...
  const [measurementTypes, setMeasurementTypes] = useState<MeasurementTypeGetDto[]>([]);
  const [loading, setLoading] = useState(true);

  const [addSizeOpen, setAddSizeOpen] = useState(false);
  const [addMeasurementOpen, setAddMeasurementOpen] = useState(false);
  const [selectedProductSizeId, setSelectedProductSizeId] = useState<number | null>(null);
  ...
  const addMeasurementForm = useForm<{ measurementTypeId: string; value: number }>({
    initialValues: { measurementTypeId: "", value: 0 },
    validate: {
      measurementTypeId: (value) => value.length <= 0 ? "Measurement type is required" : null,
  ...
  const submitAddSize = async (values: { sizeId: string; stock: number }) => {
    try {
      ...
      if (response.data.hasErrors) {
        showNotification({ message: response.data.errors?.[0]?.message ?? "Error adding size.", color: "red" });
        return;
      }
  ...
  const submitAddMeasurement = async (values: { measurementTypeId: string; value: number }) => {
    if (!selectedProductSizeId) return;
    try {
      const response = await api.post<ApiResponse<any>>(`/api/productsizemeasurements`, {
        productSizeId: selectedProductSizeId,
        measurementTypeId: parseInt(values.measurementTypeId),
        value: values.value,
      });
      if (response.data.hasErrors) {
        showNotification({ message: response.data.errors?.[0]?.message ?? "Error adding measurement.", color: "red" });
        return;
      }
  ...
  return (
      <Container>
        <Breadcrumbs mb="md">
          ...
          <Anchor onClick={() => navigate(`/categories/${product.categoryId}`)} style={{ cursor: "pointer" }}>
            Category
          </Anchor>
        ...
        <Grid gutter="xl">
          ...
          <Grid.Col span={{ base: 12, sm: 5 }}>
            ...
            >
              <Text c="dimmed" size="sm">Product image coming soon</Text>
            </Card>
          </Grid.Col>
          ...
          <Grid.Col span={{ base: 12, sm: 7 }}>
            ...
            <Text fw={500} size="lg" mb="xl">${product.price.toFixed(2)}</Text>

            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
              <Text fw={500}>Sizes and measurements</Text>
              <Button size="xs" variant="outline" onClick={() => setAddSizeOpen(true)}>
                + Add size
              </Button>
            </div>
              ...
              product.sizes.map((size) => (
                <Card key={size.id} withBorder radius="md" padding="md" mb="sm">
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
                    ...
                    <Badge variant="light" color={size.stock > 0 ? "green" : "red"}>
                    ...
                    <Table fz="xs">
                      ...
                      <Table.Tbody>
                        {size.measurements.map((m) => (
                          <Table.Tr key={m.id}>
                            <Table.Td c="dimmed">{m.measurementTypeName}</Table.Td>
                            <Table.Td>{m.value} {m.measurementTypeUnit}</Table.Td>
                          </Table.Tr>
                        ))}
                      </Table.Tbody>
                    </Table>
        ...
        <Modal
          opened={addSizeOpen}
          onClose={() => { setAddSizeOpen(false); addSizeForm.reset(); }}
        ...
        >
          <form onSubmit={addSizeForm.onSubmit(submitAddSize)}>
            ...
            />
            <Button type="submit" mt="md" fullWidth>Add size</Button>
          </form>
        </Modal>

        <Modal
          opened={addMeasurementOpen}
          onClose={() => { setAddMeasurementOpen(false); addMeasurementForm.reset(); }}
        ...
        >
          <form onSubmit={addMeasurementForm.onSubmit(submitAddMeasurement)}>
            <Select
              ...
              data={measurementTypes.map((mt) => ({ value: String(mt.id), label: `${mt.name} (${mt.unit})` }))}
            ...
            />
            <Button type="submit" mt="md" fullWidth>Add measurement</Button>
          </form>
        </Modal>
